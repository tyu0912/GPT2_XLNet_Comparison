...
# In[ ]:
model.load_weights('model_checkpoint_LSTM_character_Angela_attemp2-084.hdf5')

checkpoint_path = "model_checkpoint_LSTM_character_Angela_attemp3-{epoch:03d}.hdf5"
...
history = model.fit(train_X, train_Y, epochs=50, batch_size=2560, validation_data=(val_X,val_Y), callbacks=desired_callbacks)
model.save('model_LSTM_character_Angela_attemp3.hdf5') 
...
np.savetxt("model_LSTM_character_Angela_attemp3_loss_history_train.txt", np.array(train_loss), delimiter=",")
np.savetxt("model_LSTM_character_Angela_attemp3_loss_history_val.txt", np.array(val_loss), delimiter=",")
